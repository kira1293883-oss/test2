...
TriggerF11Register() {
    ...
    try {
        ...
        lvCyclus.OnEvent("DoubleClick", KlikkKopierKolonneF11)
...
; ==============================================================================
; 📋 DOBBELTKLIKK EN CELLE I REGISTERET FOR Å KOPIERE NØYAKTIG DET SOM STÅR DER
; — akkurat som i Excel: hva du enn dobbeltklikker (Filnavn, Passord, URL),
...
KlikkKopierKolonneF11(ctrl, rad) {
    ...
    try {
        ...
        ; Finn hvilken kolonne musepekeren er over. LVM_SUBITEMHITTEST kan være
        ; upålitelig avhengig av systemets comctl32-versjon, så vi regner i
        ; stedet ut kolonnen direkte ut fra faktisk kolonnebredde (mer robust).
        MouseGetPos(&mx, &my)
        pt := Buffer(8, 0)
        NumPut("int", mx, pt, 0)
        NumPut("int", my, pt, 4)
        DllCall("ScreenToClient", "ptr", ctrl.Hwnd, "ptr", pt)
        klientX := NumGet(pt, 0, "int")

        col1Bredde := SendMessage(0x101D, 0, 0, ctrl.Hwnd)   ; LVM_GETCOLUMNWIDTH, kolonne 0
        col2Bredde := SendMessage(0x101D, 1, 0, ctrl.Hwnd)   ; kolonne 1

        if (klientX < col1Bredde)
            kolonne := 0
        else if (klientX < col1Bredde + col2Bredde)
            kolonne := 1
        else
            kolonne := 2

        element := CyclusKart[rad]
        navn := ["Filnavn", "Passord", "URL"]
        hva := navn[kolonne + 1]

        ; Nekt kopiering av "ikke satt enda"-tilstander (venter på passord / ingen URL)
        if (kolonne = 1 and (element.p = "VENTER_PÅ_MATING" or element.p = "")) {
            ToolTip("⚠️ Denne filen har ikke fått passord enda!")
            SetTimer(() => ToolTip(), -1800)
            return
        }
        if (kolonne = 2 and (!element.HasOwnProp("url") or element.url = "")) {
            ToolTip("⚠️ Ingen URL satt for denne filen enda!")
            SetTimer(() => ToolTip(), -1800)
            return
        }

        ; Hent EKSAKT teksten som vises i cellen (samme prinsipp som Excel:
        ; det du dobbeltklikker på er det du får kopiert).
        verdi := ctrl.GetText(rad, kolonne + 1)
