...
; 🖥️ DPI-AWARENESS: Må settes FØR noe vindu opprettes. Uten dette DPI-
; virtualiserer Windows koordinater ulikt for ClientToScreen (fysiske piksler)
; og Gui.Show (virtualiserte piksler), som gir feilplasserte popup-bokser
; (f.eks. tekstmarkeringen i F11-registeret) på skjermer med skalering != 100%.
try DllCall("SetProcessDpiAwarenessContext", "ptr", -4, "int")   ; PER_MONITOR_AWARE_V2 (Win10 1703+)
...
global SpdIdx := 11
global TypDelay := 0
global PostFilPause := 100
global WinRARPrioritet := "High"

; 🚀 PARALLELL UTPAKKING: hvor mange 7-Zip-arkiver som pakkes ut SAMTIDIG
; i stedet for én om gangen. Høyere tall = raskere når disk/CPU har kapasitet,
; men bruker mer RAM/CPU/disk-IO samtidig. Juster i F7-panelet (slider) eller
; her. 1 = gammel oppførsel (én fil av gangen, helt sekvensielt).
; 🧠 CPU-TILPASSET STANDARDVERDI: i stedet for en fast "3" for alle, les
; antall logiske CPU-kjerner og sett en fornuftig standard ut fra det
; (halvparten av kjernene, minst 2, maks 8 — 7-Zip-utpakking er delvis
; disk-IO-bundet så vi vil ikke bruke ALLE kjernene til dette alene).
BeregnStandardParallellitetF3() {
    try {
        antallKjerner := 4
        for cpu in ComObjGet("winmgmts:").ExecQuery("SELECT NumberOfLogicalProcessors FROM Win32_ComputerSystem")
            antallKjerner := cpu.NumberOfLogicalProcessors
        forslag := Max(2, Min(8, Round(antallKjerner / 2)))
        return forslag
    } catch {
        return 3
    }
}
global MaxSamtidigUtpakking := BeregnStandardParallellitetF3()
...
global HK_Notepad := "^!n"
global HK_Folder := "^!f"
global HK_UrlMeny := "F6"
global HK_ChromeRotate := "^!g"
global HK_ChromeCount := "^!w"
global HK_ChromePaste := "!p"
...
Hotkey(HK_Feed, (*)=>MaterPassordF11())
Hotkey(HK_Notepad, (*)=>ÅpnePassordNotatF11())
Hotkey(HK_Folder, (*)=>F5_EndreMappeLive())
Hotkey(HK_UrlMeny, (*)=>TriggerF6UrlListeMeny())
Hotkey(HK_ChromeRotate, (*)=>SendUrlTilChromeRotererF11())
Hotkey(HK_ChromeCount, (*)=>VisAntallChromeVinduerF11())
Hotkey(HK_ChromePaste, (*)=>SendValgtTekstAltP())

; Velkomstmelding ved oppstart
try {
    MsgBox("🤖 MODULÆRT DELUXE SYSTEM STARTET!`n`n🚀 TURBO 0MS AKTIVERT SOM STANDARD!`n`nF1: Knapper | F3: Pakk ut | F5: Auto-Close | F6: Bannliste | F7: Fart | F10: Tema | F11: Passordregister`nAlt+P: Send merket lenke til Chrome (roterer på alle åpne vinduer)`nCtrl+Alt+W: Vis antall åpne Chrome-vinduer", "System", 64)
}
...
F5:: {
    ...
    try {
        ...
        sysGui.SetFont("s8.5 cAAAAAA Norm")
        kortSti := (StrLen(SrcM) > 42) ? SubStr(SrcM, 1, 40) "..." : SrcM
        ...
        sysGui.SetFont("s8 c888888 Norm")
