            ...
            for j, jobb in aktiveJobber {
                ...
                try {
                    if (ok) {
                        ...
                        LagreAutoPassordF11(jobb.nam, jobb.p, denneRunden, (jobb.element.HasOwnProp("url") ? jobb.element.url : ""), "FEILET (feil passord eller korrupt arkiv)")
                        if (LydVarslingPå)
                            TrayTip("Failed!", jobb.nam, 3)
                    }
                } catch as e {
                    ; 🩹 FIKS: en feil her (f.eks. Notepad-loggfila låst av et
                    ; annet program akkurat i dette øyeblikket) ble tidligere
                    ; svelget helt stille av en tom try-blokk uten catch — man
                    ; fikk aldri vite at en fil manglet fra loggen. Nå skrives
                    ; det i alle fall til utpakkingsloggen (LogF), slik at det
                    ; er synlig i etterkant, og resten av køen fortsetter.
                    try FileAppend("FIL: " jobb.nam " -> ⚠️ Klarte ikke skrive til Notepad-loggen: " e.Message "`n", LogF, "UTF-8")
                }
