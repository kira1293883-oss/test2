...
TriggerF1Knappeoppsett() {
    global HK_Pause, HK_Extract, HK_Gui, HK_Speed, HK_Lock, HK_Round, HK_SaveNote, HK_Link, HK_Feed, HK_Notepad, HK_Folder, HK_UrlMeny, HK_ChromeRotate, HK_ChromeCount, HK_ChromePaste, cfg
    ...
    try {
        ...
        cfg.Add("Text", "w120 xm y+10", "URL-/Bannliste:")
        iUrlMeny := cfg.Add("Edit", "w140 x+10 r1 +Background333333 cWhite", HK_UrlMeny)
        ...
        cfg.SetFont("s9 cCCCCCC Bold", "Segoe UI")
        cfg.Add("Text", "w270 xm y+18", "🌐 CHROME-SNARVEIER:")
        cfg.SetFont("s9 cAAAAAA w400", "Segoe UI")
        cfg.Add("Text", "w120 xm y+12", "Send lenke (rotér):")
        iChromeRotate := cfg.Add("Edit", "w140 x+10 r1 +Background333333 cWhite", HK_ChromeRotate)
        cfg.Add("Text", "w120 xm y+10", "Antall vinduer:")
        iChromeCount := cfg.Add("Edit", "w140 x+10 r1 +Background333333 cWhite", HK_ChromeCount)
        cfg.Add("Text", "w120 xm y+10", "Send merket tekst:")
        iChromePaste := cfg.Add("Edit", "w140 x+10 r1 +Background333333 cWhite", HK_ChromePaste)

        cfg.SetFont("s9 cWhite Bold")
        cfg.Add("Button", "w270 h32 xm y+20 -Theme +Background3A3A3A", "💾 Lagre Knapper").OnEvent("Click", (btn, *)=>LagreNyeHurtigtaster(cfg, iP.Value, iE.Value, iG.Value, iFart.Value, iLock.Value, iRound.Value, iSave.Value, iLink.Value, iFeed.Value, iNotepad.Value, iFolder.Value, iUrlMeny.Value, iChromeRotate.Value, iChromeCount.Value, iChromePaste.Value))
        cfg.Add("Button", "w270 h32 xm y+10 -Theme +Background3A3A3A", "🔄 Reset Standard").OnEvent("Click", (btn, *)=>LagreNyeHurtigtaster(cfg, "F2", "F3", "F11", "F7", "^!l", "^!r", "^!s", "^!u", "^+c", "^!n", "^!f", "F6", "^!g", "^!w", "!p"))
