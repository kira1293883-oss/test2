...
; ==============================================================================
; 🩹 FIKS: TRYGG SKRIVING TIL UTPAKKINGSLOGGEN (LogF)
;
; PROBLEM: Ved flere samtidige utpakkinger kan flere jobber bli ferdig nesten
; i samme øyeblikk, og alle skriver da til samme LogF-fil rett etter
; hverandre. Rammes filen av en kortvarig ekstern lås (AV-skanning, OneDrive/
; sky-synk, filindeksering) akkurat da, kastet den vanlige FileAppend() et
; unntak — og siden den satt i SAMME try-blokk som selve lagringen av
; passord/URL/navn (LagreAutoPassordF11 / LagrePassordEgenFilF11), hoppet
; koden rett til catch FØR de rekk å kjøre. Resultat: jo flere filer som ble
; ferdig samtidig, jo oftere "forsvant" passord/URL/navn for akkurat den
; filen — helt stille, uten feilmelding.
;
; LØSNING: samme retry-mønster som SkrivTilPassordLoggF11 i Main_F11Register.ahk,
; brukt for ALLE skriv til LogF i utpakkingsmotoren. Denne kaster ALDRI et
; unntak videre, så den kan aldri avbryte try-blokken den står i.
; ==============================================================================
SkrivTilUtpakkingsLoggF3(tekst, maxForsøk := 5, ventMs := 100) {
    global LogF
    Loop maxForsøk {
        try {
            FileAppend(tekst, LogF, "UTF-8")
            return true
        } catch {
            if (A_Index = maxForsøk) {
                ToolTip("⚠️ Utpakkingsloggen var låst — fikk ikke skrevet en linje dit (passord/URL ble likevel forsøkt lagret separat).")
...
; ==============================================================================
; Synkende sortering (størst→minst) på "_størrelse"-egenskapen.
; Brukt til å planlegge parallell-utpakking mer effektivt (se TriggerUtpakkingLive).
; ⚡ MERGE SORT: O(n log n) i stedet for den gamle O(n²) boble-sorteringen.
; Boble-sort var helt fint for noen titalls filer, men skalerer dårlig — med
; f.eks. 500 arkiv i køen gjør O(n²) ca. 250 000 sammenligninger mot merge
; sorts ca. 4 500, så dette holder køen rask å planlegge selv for store batcher.
; Stabil (bevarer innbyrdes rekkefølge for filer med lik størrelse).
; ==============================================================================
SorterKøEtterStørrelseF3(liste) {
    ...
    høyre := []
    Loop midt
        venstre.Push(liste[A_Index])
    Loop n - midt
        høyre.Push(liste[midt + A_Index])

    venstre := SorterKøEtterStørrelseF3(venstre)
    høyre := SorterKøEtterStørrelseF3(høyre)

    resultat := []
    i := 1, j := 1
    while (i <= venstre.Length and j <= høyre.Length) {
        if (venstre[i]._størrelse >= høyre[j]._størrelse) {
            resultat.Push(venstre[i])
            i++
        } else {
            resultat.Push(høyre[j])
            j++
        }
    }
...
TriggerUtpakkingLive() {
    ...
    ferdigeFiler := []
    lagringsLogg := []   ; 🆘 FAILSAFE: {nam, p, url, ok, lagret} for HVER fullført fil denne runden
    ...
    try {
        while (fullførte < tot) {
            ...
            while (aktiveJobber.Length < maxSamtidig and nesteIdx <= tot) {
                ...
                try {
                    ...
                    ; 🔍 DIAGNOSE-LOGGING FJERNET: skrev tidligere passordlengde
                    ; + første/siste tegn til Vellykkede_Passord.txt (LogF) for
                    ; HVER fil rett før hver 7-Zip-oppstart. Det var et per-fil
                    ; FileAppend midt i den parallelle hot-loopen som la på
                    ; unødvendig disk-I/O og saktnet ned hele utpakkingsrunden.
                    ; Fjernet for fart — sett den tilbake midlertidig hvis du
                    ; trenger å feilsøke et spesifikt passordproblem igjen.
            ...
            if (aktiveJobber.Length = 0 and nesteIdx > tot and fullførte < tot) {
                SkrivTilUtpakkingsLoggF3("ADVARSEL: Utpakkingskøen kom i utakt (" fullførte "/" tot " talt, ingen aktive jobber igjen) — resterende " (tot - fullførte) " fil(er) merkes FEILET for å unngå at systemet henger.`n")
                while (fullførte < tot) {
            ...
            for j, jobb in aktiveJobber {
                ...
                if (jobb.pid != 0 and ProcessExist(jobb.pid) and (A_TickCount - jobb.startTick) > 20*60*1000) {
                    ...
                    SkrivTilUtpakkingsLoggF3("ADVARSEL: " jobb.nam " ble tvunget avsluttet etter 20 minutter uten fremgang (mulig hengende/skadet arkiv).`n")
                }
                ...
                if (!ok and !(jobb.element.HasOwnProp("_retry") and jobb.element._retry)) {
                    ...
                    SkrivTilUtpakkingsLoggF3("FIL: " jobb.nam " -> feilet første forsøk, prøver automatisk på nytt...`n")
                    koe.Push(jobb.element)
                    tot++
                    continue
                }

                ; 🩹 FIKS: LOGGSKRIVING OG PASSORD/URL-LAGRING KJØRER NÅ I HELT
                ; ADSKILTE try-BLOKKER. Tidligere lå BEGGE i samme try-blokk,
                ; så hvis FileAppend til LogF kastet et unntak (f.eks. fila var
                ; kortvarig låst av AV/sky-synk — mer sannsynlig jo flere filer
                ; som blir ferdige samtidig under parallell utpakking), hoppet
                ; koden rett til catch og LagreAutoPassordF11/LagrePassordEgenFilF11
                ; ble ALDRI kalt for den filen. Passord, URL og navn "forsvant"
                ; da helt stille. Nå er de to heltuavhengige av hverandre, og
                ; selve LogF-skrivingen bruker den robuste retry-skriveren i
                ; stedet for en rå FileAppend som kunne kaste et unntak.
                if (ok) {
                    SkrivTilUtpakkingsLoggF3("FIL: " jobb.nam " -> OK med tilordnet passord`n")
                    lagretOk := false
                    try {
                        lagretOk := LagreAutoPassordF11(jobb.nam, jobb.p, denneRunden, (jobb.element.HasOwnProp("url") ? jobb.element.url : ""))
                        LagrePassordEgenFilF11(jobb.nam, jobb.p)
                    } catch as e {
                        SkrivTilUtpakkingsLoggF3("FIL: " jobb.nam " -> ⚠️ Klarte ikke lagre passord/URL: " e.Message "`n")
                    }
                    lagringsLogg.Push({nam: jobb.nam, p: jobb.p, url: (jobb.element.HasOwnProp("url") ? jobb.element.url : ""), status: "OK med tilordnet passord", lagret: lagretOk})
                    if (LydVarslingPå)
                        TrayTip("Saved!", jobb.nam, 1)
                } else {
                    SkrivTilUtpakkingsLoggF3("FIL: " jobb.nam " -> FEILET (feil passord eller korrupt arkiv, etter automatisk retry)`n")
                    ; 🩹 FIKS: Notepad-loggens hode lover at BÅDE OK og FEILET
                    ; blir auto-lagret dit, men her ble bare "OK"-tilfellet
                    ; faktisk skrevet til Notepad-loggen. Kaller nå
                    ; LagreAutoPassordF11 også for feilede filer, slik at
                    ; loggen faktisk holder det den lover.
                    lagretOk := false
                    try {
                        lagretOk := LagreAutoPassordF11(jobb.nam, jobb.p, denneRunden, (jobb.element.HasOwnProp("url") ? jobb.element.url : ""), "FEILET (feil passord eller korrupt arkiv)")
                    } catch as e {
                        SkrivTilUtpakkingsLoggF3("FIL: " jobb.nam " -> ⚠️ Klarte ikke skrive til Notepad-loggen: " e.Message "`n")
                    }
                    lagringsLogg.Push({nam: jobb.nam, p: jobb.p, url: (jobb.element.HasOwnProp("url") ? jobb.element.url : ""), status: "FEILET (feil passord eller korrupt arkiv)", lagret: lagretOk})
                    if (LydVarslingPå)
        ...
        ; ==========================================================================
        ; 🆘 FAILSAFE — SLUTT-AV-RUNDEN-AVSTEMMING
        ; Går gjennom lagringsLogg (ALT som ble fullført denne runden, uansett
        ; om det ble lagret riktig underveis) og gir alt som IKKE ble bekreftet
        ; lagret ett ekstra forsøk nå — de kortvarige låsene (AV/sky-synk) som
        ; er hovedårsaken til feil, er nesten alltid over i løpet av rundens
        ; varighet. Om noe FORTSATT ikke lar seg lagre, dumpes det til en egen,
        ; garantert fersk oppsummeringsfil for akkurat denne runden, slik at et
        ; passord/URL/navn ALDRI kan forsvinne sporløst.
        ; ==========================================================================
        ikkeReddet := []
        for idx, rad in lagringsLogg {
            if (rad.lagret)
                continue
            reddet := false
            try
                reddet := LagreAutoPassordF11(rad.nam, rad.p, denneRunden, rad.url, rad.status)
            if !reddet
                ikkeReddet.Push(rad)
        }
        if (ikkeReddet.Length > 0) {
            oppsumSti := A_Desktop "\Runde" denneRunden "_Sikkerhetskopi_Passord.txt"
            tekst := "=== SIKKERHETSKOPI — RUNDE " denneRunden " ===`r`n"
            tekst .= "Disse " ikkeReddet.Length " linjene klarte IKKE hovedloggen eller nødfila å lagre underveis,`r`n"
            tekst .= "så de er dumpet her som siste sikkerhetsnett. Kopier dem til Lagrede_Passord.txt manuelt.`r`n`r`n"
            for idx, rad in ikkeReddet {
                tekst .= "FIL: " rad.nam " -> " rad.status " | Passord: " rad.p
                if (rad.url != "")
                    tekst .= " | URL: " rad.url
                tekst .= "`r`n"
            }
            try FileAppend(tekst, oppsumSti, "UTF-8")
        }
        ...
        if (ikkeReddet.Length > 0)
            meldingTekst .= "`n`n🆘 " ikkeReddet.Length " passord kunne IKKE lagres i hovedloggen selv etter gjentatte forsøk — de er reddet til en egen fil på skrivebordet: Runde" denneRunden "_Sikkerhetskopi_Passord.txt. Sjekk den filen!"
