...
VisAntallChromeVinduerF11() {
    ...
    ToolTip("🌐 " antall " Chrome-vindu(er) åpen(t) akkurat nå (roterer gjennom alle). Neste i rotasjon: vindu " ChromeRotasjonsIdx "/" antall)
...
SendUrlTilChromeRotererF11() {
    ...
    ; --- Bruk ALLE Chrome-vinduer som faktisk er åpne akkurat nå. Har du flere
    ; vinduer åpne enn ChromeRotasjonsMaks, blir de også brukt i rotasjonen —
    ; ChromeRotasjonsMaks fungerer nå kun som et MINSTEMÅL: er det færre enn
    ; det åpne, åpnes nye slik at det alltid finnes et visst antall å rotere
    ; mellom. Er det allerede flere, roteres det gjennom ALLE sammen. ---
    chromeVinduer := SorterVindusListeStabiltF11(WinGetList("ahk_exe chrome.exe"))
    ...
    ; --- Velg neste vindu i rotasjonen (1..antallVinduer, går rundt til 1 igjen) ---
    if (ChromeRotasjonsIdx > antallVinduer)
        ChromeRotasjonsIdx := 1
    målHwnd := chromeVinduer[ChromeRotasjonsIdx]
    valgtIdx := ChromeRotasjonsIdx
    ChromeRotasjonsIdx := (ChromeRotasjonsIdx >= antallVinduer) ? 1 : ChromeRotasjonsIdx + 1
    ...
    try {
        ...
        Sleep(80)
        Send("{Enter}")
        ToolTip("🌐 Sendt til Chrome-vindu " valgtIdx "/" antallVinduer ": " SubStr(lenke, 1, 50))
        SetTimer(() => ToolTip(), -2000)
    } catch as e {
