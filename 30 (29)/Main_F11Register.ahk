...
; ==============================================================================
; 🩹 FIKS: SIKKER SKRIVING TIL NOTEPAD-LOGGEN (PassordNotatFil)
;
; PROBLEM: Ved rask/parallell utpakking (flere filer ferdig nesten samtidig)
; kunne FileAppend til PassordNotatFil av og til feile fordi fila var kortvarig
; LÅST — av AV-skanning, OneDrive/skylagring-synk, filindeksering, eller en
; Notepad-instans som allerede har fila åpen. ALLE tidligere skrive-steder
; brukte en TOM "try { FileAppend(...) }" UTEN catch, så en slik forbigående
; lås gjorde at linja (filnavn + passord + URL) rett og slett aldri ble
; skrevet — helt stille, uten noen feilmelding. Jo raskere/mer parallelt
; utpakkingen kjørte, jo oftere traff man en slik kortvarig lås, og jo flere
; linjer "forsvant" fra loggen.
;
; LØSNING: denne felles hjelpefunksjonen brukes nå av ALLE steder som skriver
; til PassordNotatFil. Den prøver flere ganger med en kort pause mellom hvert
; forsøk (en typisk AV/sync-lås varer bare noen titalls millisekunder), og
; hvis ALLE forsøk feiler, skrives det i det minste en tydelig ⚠️-varsling til
; utpakkingsloggen (LogF) i stedet for å forsvinne sporløst. ---
; ==============================================================================
SkrivTilPassordLoggF11(tekst, maxForsøk := 8, ventMs := 150) {
    global PassordNotatFil
    Loop maxForsøk {
        try {
            FileAppend(tekst, PassordNotatFil, "UTF-8")
            return true
        } catch as e {
            if (A_Index = maxForsøk) {
                ; --- Siste forsøk feilet også — varsle i stedet for å tie. Vi
                ; prøver å skrive varselet til LogF (utpakkingsloggen) hvis den
                ; finnes, ellers vises en ToolTip så det i alle fall er synlig
                ; akkurat nå. ---
                global LogF
                varsel := "⚠️ KLARTE IKKE Å SKRIVE TIL NOTEPAD-LOGGEN (" e.Message ") etter " maxForsøk " forsøk — mistet linje: " tekst
                try {
                    if (IsSet(LogF) and LogF != "")
                        FileAppend(varsel "`n", LogF, "UTF-8")
                }
                ToolTip("⚠️ Notepad-loggen var låst — fikk IKKE lagret en linje. Se utpakkingsloggen.")
                SetTimer(() => ToolTip(), -4000)
                return false
            }
            ; --- Ikke siste forsøk ennå: vent kort og prøv igjen. Fila er
            ; nesten alltid ledig igjen etter noen titalls/hundre ms. ---
            Sleep(ventMs)
        }
    }
    return false
...
global CyclusVinduX        := ""   ; Husker X-posisjon F11-panelet sist ble flyttet/lukket på
global CyclusVinduY        := ""   ; Husker Y-posisjon F11-panelet sist ble flyttet/lukket på
...
; ==============================================================================
TriggerF11Register() {
    global cyclusGui, lvCyclus, CyclusSkannIntervallms, Themes, StlIdx, lockBtnF11, LåstPassordAktiv, LåstPassordVerdi, urlToggleBtnF11, LagreUrlAktiv, CyclusVinduX, CyclusVinduY
    try {
        if IsObject(cyclusGui) and WinExist("ahk_id " cyclusGui.Hwnd) {
            LukkF11RegisterPanel()
            return
        }
        st := Themes[StlIdx]
        ...
        cyclusGui.Add("Button", "x20 y488 w580 h32 -Theme +Background222222", "✖ Lukk").OnEvent("Click", (*) => LukkF11RegisterPanel())

        lvCyclus.OnEvent("Click", KlikkKopierKolonneF11)

        cyclusGui.OnEvent("Close",  (*) => LukkF11RegisterPanel())
        cyclusGui.OnEvent("Escape", (*) => LukkF11RegisterPanel())

        ; 🖱️ FLYTTBART VINDU: uten tittellinje (-Caption) kan vinduet normalt
        ; ikke flyttes med musa. Denne håndteringen lar deg klikke og dra i
        ; selve panelbakgrunnen (utenfor knapper/ListView) for å flytte det.
        OnMessage(0x0201, WM_LBUTTONDOWN_F11REGISTER)

        OppdatertCyclusPanelLiveF11()

        ; --- HUSK POSISJON: Bruker forrige lagrede X/Y hvis panelet har vært
        ; åpnet og flyttet før i denne AHK-økten. Ellers sentreres det som før
        ; (første gang det åpnes). ---
        if (CyclusVinduX != "" and CyclusVinduY != "")
            cyclusGui.Show("x" CyclusVinduX " y" CyclusVinduY " W620 H540")
        else
            cyclusGui.Show("W620 H540 Center")
    }
}

; --- Lukker F11-panelet, men husker posisjonen det sto i FØRST, slik at
; neste gang det åpnes (selv etter å ha vært helt lukket) dukker det opp på
; nøyaktig samme sted i stedet for å hoppe tilbake til midten av skjermen. ---
LukkF11RegisterPanel(*) {
    global cyclusGui, CyclusVinduX, CyclusVinduY
    try {
        if IsObject(cyclusGui) and WinExist("ahk_id " cyclusGui.Hwnd) {
            WinGetPos(&x, &y, , , "ahk_id " cyclusGui.Hwnd)
            CyclusVinduX := x
            CyclusVinduY := y
        }
    }
    try {
        if IsObject(cyclusGui) {
            cyclusGui.Destroy()
            cyclusGui := ""
        }
    } catch {
...
; ==============================================================================
; Lar F11-registervinduet flyttes ved å klikke og dra i selve bakgrunnen,
; siden det mangler tittellinje (-Caption). Fyrer KUN når klikket treffer
; selve vinduet direkte (ikke knapper/ListView, som håndterer klikk selv).
; ==============================================================================
WM_LBUTTONDOWN_F11REGISTER(wParam, lParam, msg, hwnd) {
    global cyclusGui
    try {
        if (IsObject(cyclusGui) and hwnd = cyclusGui.Hwnd)
            PostMessage(0x00A1, 2, 0, , "ahk_id " cyclusGui.Hwnd)
    }
}
...
; ==============================================================================
; 🆘 SISTE UTVEI: NØD-FALLBACK FIL
;
; Hvis SkrivTilPassordLoggF11 mislykkes ETTER alle sine retry-forsøk (en
; svært vedvarende lås — f.eks. Notepad-loggen åpen og låst av et annet
; program over lengre tid), er dette siste sikkerhetsnett: skriv linja til en
; HELT SEPARAT fil på skrivebordet som ikke brukes til noe annet, slik at et
; passord/URL/navn ALDRI kan gå fullstendig tapt selv i verste fall. Denne
; filen har sin egen uavhengige retry-runde, så selv om HOVEDLOGGEN er låst,
; er sjansen svært liten for at OGSÅ nødfila er låst av akkurat samme grunn.
; ==============================================================================
global NødLoggFil := A_Desktop "\Nødlagring_Passord.txt"
SkrivTilNødLoggF11(tekst, maxForsøk := 5, ventMs := 150) {
    global NødLoggFil
    Loop maxForsøk {
        try {
            FileAppend(tekst, NødLoggFil, "UTF-8")
            return true
        } catch {
            Sleep(ventMs)
        }
    }
    ; Selv nødfila feilet etter flere forsøk — svært uvanlig. Siste varsel er
    ; en vedvarende ToolTip, slik at det i det minste er synlig akkurat nå.
    ToolTip("🆘 KRITISK: Klarte ikke lagre passord i det hele tatt (verken hovedlogg eller nødfil)! " tekst)
...
; Returnerer true/false — kalleren kan nå faktisk vite om lagringen lyktes,
; og selv slå på nød-fallback hvis ikke (se failsafe-forklaring ved
; SkrivTilNødLoggF11 lenger opp i fila).
; ==============================================================================
LagreAutoPassordF11(navn, passord, runde, url := "", status := "OK med tilordnet passord") {
    ...
    if !FileExist(PassordNotatFil)
        SkrivTilPassordLoggF11("=== LAGREDE PASSORD (auto-lagret ved hver utpakking, OK og FEILET) ===`r`n")
    if !RundeHarSkrevetHode.Has(runde) {
        SkrivTilPassordLoggF11("`r`n=== RUNDE " runde " ===`r`n")
        RundeHarSkrevetHode[runde] := true
    }
    linje := "FIL: " navn " -> " status " | Passord: " passord
    if (LagreUrlAktiv and url != "")
        linje .= " | URL: " url
    ; 🩹 FIKS: bruker nå SkrivTilPassordLoggF11 (med automatiske retry-forsøk
    ; og ekte feilvarsling) i stedet for en tom try/FileAppend som stille
    ; mistet linja ved rask/parallell utpakking (se forklaring ved
    ; SkrivTilPassordLoggF11 lenger opp i fila).
    lyktes := SkrivTilPassordLoggF11(linje "`r`n")
    ; 🆘 FAILSAFE: hovedloggen mislyktes selv etter alle retry-forsøk —
    ; skriv umiddelbart til den uavhengige nødfila i stedet for å bare
    ; varsle og gi opp. Dette er den siste sikkerhetsnetten.
    if !lyktes
        lyktes := SkrivTilNødLoggF11("RUNDE " runde " | " linje "`r`n")
    return lyktes
}

; ==============================================================================
; 📁 LAGRE ÉN SEPARAT PASSORD-FIL PER UTPAKKET FIL — kalles fra
; utpakkingsmotoren (F3) for HVER fil som blir korrekt pakket ut. I motsetning
; til LagreAutoPassordF11 (som append'er til én stor delt logg), lager denne
; en helt egen .txt per fil, i sin egen mappe (PassordFilMappe), med kun
; passordet for akkurat den filen.
; ==============================================================================
LagrePassordEgenFilF11(navn, passord) {
    global PassordFilMappe, LogF
    try {
        if !DirExist(PassordFilMappe)
            DirCreate(PassordFilMappe)

        ; Unngå at et arkiv med samme navn som et tidligere overskriver
        ; passordfila til det forrige — legg på _2, _3 osv. ved konflikt.
        målSti := PassordFilMappe "\" navn ".txt"
        n := 2
        while FileExist(målSti) {
            målSti := PassordFilMappe "\" navn "_" n ".txt"
            n++
        }

        ; 🩹 FIKS: samme retry-mønster som SkrivTilPassordLoggF11 — hver egen
        ; fil er riktignok unik per arkiv, så kollisjon mellom to av VÅRE
        ; egne skriv er ikke mulig, men en kortvarig ekstern lås (AV-skanning
        ; av en splitter ny fil, sky-synk, osv.) kunne likevel få den tomme
        ; try-blokka til å svelge feilen stille. Prøver nå flere ganger og
        ; varsler i loggen hvis det fortsatt feiler.
        lagret := false
        Loop 8 {
            try {
                FileAppend(passord, målSti, "UTF-8")
                lagret := true
                break
            } catch {
                Sleep(150)
        ...
        ; 🆘 FAILSAFE: selv den egne passordfila mislyktes etter alle forsøk —
        ; ikke bare varsle og gi opp, skriv passordet til den uavhengige
        ; nødfila også, slik at det uansett finnes lagret et sted.
        if !lagret
            lagret := SkrivTilNødLoggF11("FIL: " navn " -> ⚠️ Egen passordfil (" målSti ") feilet, lagret her i stedet | Passord: " passord "`r`n")
        if !lagret
            try FileAppend("FIL: " navn " -> ⚠️ Klarte ikke skrive egen passord-fil (" målSti ")`n", LogF, "UTF-8")
        return lagret
    }
    return false
}
...
ÅpnePassordNotatF11() {
    ...
    try {
        if !FileExist(PassordNotatFil)
            SkrivTilPassordLoggF11("=== LAGREDE PASSORD (auto-lagret ved vellykket utpakking) ===`r`n")
...
LagreLenkeF11() {
    ...
    try {
        if !FileExist(PassordNotatFil)
            SkrivTilPassordLoggF11("=== LAGREDE PASSORD (auto-lagret ved vellykket utpakking) ===`r`n")
        SkrivTilPassordLoggF11("LENKE: " navn " -> " lenkeTekst "`r`n")
        Run('notepad.exe "' PassordNotatFil '"')
        SoundPlay("*64")
    ...
    for idx, element in CyclusKart {
        if (element.f == valgtFilnavn) {
            ...
            try {
                ...
                SkrivTilPassordLoggF11(linje "`r`n")
                Run('notepad.exe "' PassordNotatFil '"')
                SoundPlay("*64")
